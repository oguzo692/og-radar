...
# --- 4. HTML ŞABLONLARI ---
# W6 için maç detayları
w6_matches = """
<div class='terminal-row'><span>takım 1 - takım 2</span><span class='highlight'>tahmin 1 ✅</span></div>
<div class='terminal-row'><span>takım 3 - takım 4</span><span class='highlight'>tahmin 2 ❌</span></div>
<hr style='border: 0; height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;'>
<div class='terminal-row'><span>Toplam Oran: 5.50</span><span>Tutar: 100 USD</span></div>
"""
...
w6_coupon_html = f"<div class='industrial-card' style='border-top-color: #ffae00 !important;'><div class='terminal-header' style='color:#ffae00;'>⏳ W6 KUPONU (AKTİF/YENİ)</div>{w6_matches}<span style='color:#ffae00; font-weight:bold;'>BEKLENİYOR...</span></div>"
...
w3_coupon_html = f"<div class='industrial-card' style='border-top-color: #00ff41 !important;'><div class='terminal-header' style='color:#00ff41;'>✅ W3 KUPONU (BAŞARILI)</div>{w2_matches}<span style='color:#00ff41; font-weight:bold;'>SONUÇLANDI ✅</span></div>"
...
if check_password():
    ...
    if page == "⚡ ULTRA ATAK":
        ...
        # Sekme Yapılandırması (Tüm haftalar burada toplanmıştır)
        t6, t5, t4, t3, t2, t1 = st.tabs(["🚀 W6", "❌ W5", "❌ W4", "✅ W3", "✅ W2", "❌ W1"])
        
        with t6:
            # W6 için dinamik durum belirleme
            ...
            <hr style='border: 0; height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;'>
            <div class='terminal-row'><span>Haftalık K/Z:</span><span style='color:white; font-weight:bold;'>${w6_kar}</span></div>
            """
            st.markdown(f"<div class='industrial-card' style='border-top-color: {w6_color} !important;'><div class='terminal-header' style='color:{w6_color};'>{w6_status} - WEEK 6</div>{w6_matches_content}</div>", unsafe_allow_html=True)

        # Diğer sekmeler (Mevcut HTML şablonların kullanılır)
