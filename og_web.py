...
if check_password():
    ...
    # --- 5. FORM LINE ---
        ...
        with t1:
            st.markdown("<div class='glass-card'>W3 Maçları Henüz Girilmedi.</div>", unsafe_allow_html=True)
        
        with t2:
            st.markdown("""<div class='coupon-card'>
                <h2 style='color:#f1c40f;'>⏳ W2 - BEKLEMEDE</h2><br>
                <div class='match-row'><span>GS - Kayserispor</span><span class='status-win'>✅</span></div>
                <div class='match-row'><span>Liverpool - Newcastle</span><span class='status-win'>✅</span></div>
                <div class='match-row'><span>BVB - Heidenheim</span><span class='status-win'>✅</span></div>
                <div class='match-row'><span>Kocaelispor - FB</span><span class='status-wait'>⏳</span></div>
                <hr style='border: 1px solid rgba(255,255,255,0.05); margin: 20px 0;'>
                <p><b>Toplam Oran: 5.40 | Bütçe: 100 USD</b></p>
                </div>""", unsafe_allow_html=True)
                
        with t3:
            st.markdown("""<div class='coupon-card' style='border-color:#ff4b4b;'>
                <h2 style='color:#ff4b4b;'>❌ W1 - KAYBETTİ</h2><br>
                <div class='match-row'><span>Karagümrük - GS</span><span class='status-win'>✅</span></div>
                <div class='match-row'><span>New - Aston Villa</span><span class='status-loss'>❌</span></div>
                <div class='match-row'><span>FB - Göztepe</span><span class='status-loss'>❌</span></div>
                <hr style='border: 1px solid rgba(255,255,255,0.05); margin: 20px 0;'>
                <p><b>Toplam Oran: 7.09 | Bütçe: 100 USD</b></p>
                </div>""", unsafe_allow_html=True)

    # --- 6. DASH DASH ---
    elif page == "📊 DashDash":
        st.title("📊 DashDash")
        st.markdown("<div class='glass-card'>Veri görselleştirme modülü aktif.</div>", unsafe_allow_html=True)
