...
.terminal-header { 
    color: var(--soft-orange); font-size: 14px; font-weight: bold; 
    border-bottom: 1px dashed #30363d; padding-bottom: 5px; margin-bottom: 10px; text-transform: uppercase;
}
...
.milestone { position: absolute; top: 50%; transform: translate(-50%, -50%); width: 120px; display: flex; flex-direction: column; align-items: center; }
...
.milestone.active .milestone-label { color: #00ff41; text-shadow: 0 0 5px rgba(0, 255, 65, 0.6); }
</style>
"""

# --- 3. HTML ŞABLONLARI (KUPONLAR) ---
...
# --- 4. GOOGLE SHEETS & GÜVENLİK ---
conn = st.connection("gsheets", type=GSheetsConnection)

def load_game_data():
    try:
        df = conn.read(worksheet="Sayfa1", ttl=0)
        return {"kasa": float(df.iloc[0]["kasa"]), "ana_para": float(df.iloc[0]["ana_para"]), "yakim": int(df.iloc[0]["yakim"])}
    except:
        return {"kasa": 600.0, "ana_para": 600.0, "yakim": 20}

def save_game_data():
    new_data = pd.DataFrame([{"kasa": st.session_state.kasa_input, "ana_para": st.session_state.ana_input, "yakim": st.session_state.yakim_input}])
    conn.update(worksheet="Sayfa1", data=new_data)
    st.toast("☁️ BULUT KAYDI TAMAMLANDI", icon="✅")
...
if check_password():
    ...
    if page == "⚡ ULTRA FON":
        ...
        if st.button("💾 HIZLI KAYDET", key="main_save"): save_game_data()

        st.markdown(f"<div class='industrial-card'><div class='terminal-header'>💎 OG TRADE RADAR</div><div class='terminal-row'><span>💰 KASA</span><span class='highlight'>${kasa:,.2f}</span></div><div class='terminal-row'><span>🚀 KAR</span><span style='color:{"#00ff41" if net_kar >=0 else "#ff4b4b"}'>{net_kar:,.2f} USD (%{kar_yuzdesi:.1f})</span></div></div>", unsafe_allow_html=True)
        ...
        gun_sayisi = st.slider("Süre (Gün)", 7, 120, 30)
        vals = [kasa * ((1 + h_oran/100) ** (g / 7)) for g in range(gun_sayisi)]
        st.line_chart(pd.DataFrame({"Gün": range(gun_sayisi), "Kasa Tahmini ($)": vals}).set_index("Gün"))
        st.success(f"🚀 {gun_sayisi} gün sonra tahmin: **${vals[-1]:,.2f}**")
    ...
    st.caption("OG Core v8.8 | Full Build Integrated")
