...
.auth-container {
    padding: 3rem; background: linear-gradient(145deg, rgba(15,15,15,0.98) 0%, rgba(0,0,0,1) 100%);
    border: 1px solid rgba(204, 122, 0, 0.4); text-align: center; max-width: 600px; margin: 10vh auto;
}
.auth-header { font-family: 'Orbitron', sans-serif !important; font-size: 50px; font-weight: 900; color: #ffffff; letter-spacing: 10px; margin-bottom: 20px; }
...
[data-testid="stSidebar"] [data-testid="stImage"] { text-align: center; display: block; margin: 0 auto 10px auto; }
</style>
"""
...
def check_password():
    if not st.session_state["password_correct"]:
        ...
        # GİRİŞ EKRANI BAŞLIĞI GERİ GELDİ
        st.markdown('<div class="auth-container"><div class="auth-header">OG CORE</div><div style="font-size: 10px; color: #cc7a00; letter-spacing: 5px; text-transform: uppercase; margin-bottom: 30px; opacity: 0.8;">SİSTEME ERİŞİM İÇİN ANAHTAR GİRİNİZ</div></div>', unsafe_allow_html=True)
...
if check_password():
    ...
    with st.sidebar:
        try:
            st.image("logo.png", width=80) # Logoyu Sidebar'da küçülttüm
        except:
            pass
            
        st.markdown("<h2 style='color:#cc7a00; font-family:Orbitron; letter-spacing:4px; text-align:center; margin-top:0;'>OG CORE</h2>", unsafe_allow_html=True)
    ...
    if page == "⚡ ULTRA ATAK FON":
        ...
        st.markdown(f"<div class='industrial-card' style='border-top: 1px solid rgba(255,255,255,0.05);'><div class='terminal-header'>HEDEF İLERLEME DURUMU</div><div style='background:#111; height:10px; border-radius:5px; margin-top:10px;'><div style='background:linear-gradient(90deg, #cc7a00, #ffae00); width:{current_pct}%; height:100%; border-radius:5px;'></div></div></div>", unsafe_allow_html=True)
        ...
        with col2:
            try:
                ...
                st.markdown(f"<div class='industrial-card' style='height:210px;'><div class='terminal-header'>⚡ PİYASA NABZI</div><div class='terminal-row'><span>BITCOIN</span><span>${btc['Close'].iloc[-1]:,.0f}</span></div><div class='terminal-row'><span>SOLANA</span><span>${sol['Close'].iloc[-1]:,.2f}</span></div></div>", unsafe_allow_html=True)
        ...
        if son_islemler_raw:
            for item in son_islemler_raw.split(','):
                ...
                son_islemler_html += f"<div class='terminal-row'><span>{parts[0]}</span><span style='font-weight:bold;'>{parts[1] if len(parts)>1 else ''}</span></div>"
